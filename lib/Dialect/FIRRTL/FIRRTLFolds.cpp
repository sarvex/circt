...
static LogicalResult canonicalizePrimOp(
  ...
  if (type_isa<SIntType>(type) &&
      type_isa<UIntType>(resultValue.getType()))
...
public:
  ...
  matchAndRewrite(Operation *op,
    ...
    auto pad = [&](Value input) -> Value {
      auto inputWidth = type_cast<FIRRTLBaseType>(input.getType())
...
struct AggOneShot : public mlir::RewritePattern {
  ...
  SmallVector<Value> getCompleteWrite(Operation *lhs) const {
    ...
    if (!firrtl::type_isa<BundleType, FVectorType>(lhsTy))
      return {};
