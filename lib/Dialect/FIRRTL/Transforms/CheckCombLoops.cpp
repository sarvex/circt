...
// Perform a a dataflow analysis, on the domain of FieldRefs. Interpret each
...
public:
  DiscoverLoops(
      ...
      : module(module), instanceGraph(instanceGraph),
        modulePortPaths(modulePortPaths), portPaths(portPaths) {}
      ...
      llvm::TypeSwitch<Operation *>(&op)
          ...
          .Case<SubindexOp>([&](SubindexOp sub) {
            recordValueRefersToFieldRef(
                sub.getInput(),
                sub.getInput().getType().getFieldID(sub.getIndex()),
                sub.getResult());
          })
          .Case<SubfieldOp>([&](SubfieldOp sub) {
            recordValueRefersToFieldRef(
                sub.getInput(),
                sub.getInput().getType().getFieldID(sub.getFieldIndex()),
                sub.getResult());
          })
  ...
  std::string getName(FieldRef v, size_t f = 0) {
    return getFieldName(v).first;
  };
  ...
  // if not already existing. Then add an edge from `src` to `dst`. If
  // `recordOutput` is true, then `dst` is an output port.
  void addReachingDef(FieldRef dst, FieldRef src, bool recordOutput = false) {
      ...
      if (dstValType && !dstValType.isGround())
        walkGroundTypes(dstValType, [&](uint64_t dstIndex, FIRRTLBaseType t) {
          ...
          if (srcValType == dstValType)
            addReachingDef(dst.getSubField(dstIndex), src.getSubField(dstIndex),
                           true);
            ...
            walkGroundTypes(srcValType,
                            [&](uint64_t srcIndex, FIRRTLBaseType t) {
                              addReachingDef(dst.getSubField(dstIndex),
                                             src.getSubField(srcIndex), true);
                            });
          // Else, the src is ground type.
          else
            addReachingDef(dst.getSubField(dstIndex), src, true);
        });

      addReachingDef(dst, src, true);
    };

    // Both the dstVal and srcVal, can refer to multiple FieldRefs, ensure that
    // we capture the dataflow between each pair.
    llvm::for_each(srcFields.empty() ? srcIt->second : srcFields,
                   [&](const FieldRef &srcField) {
                     if (dstIt != valToFieldRefs.end())
                       llvm::for_each(dstIt->second,
  ...
  void handleInstanceOp(InstanceOp inst) {
    ...
    for (auto &path : modulePaths->second) {
      ...
      for (auto &modSrcPort : path.second) {
        auto srcArgNum =
            cast<BlockArgument>(modSrcPort.getValue()).getArgNumber();
        // Self loop will exist for RWProbes, ignore them.
        if (modSrcPort == modSinkPortField)
          continue;

        FieldRef srcPort(inst.getResult(srcArgNum), modSrcPort.getFieldID());
        // if (srcPort.getValue().use_empty())
        //   continue;
        ...
        if (sinkPortIsForceable && srcPortIsForceable) {
          ...
          if (rwProbeClasses.findLeader(srcNode) !=
            ...
            continue;
          // Check if sinkPort is a reaching def of sourcePath.
          auto reachingDefsToSrcPort = modulePaths->second.find(modSrcPort);
      ...
      // Make the RWProbe as the base value. Note: this is a port and
      // the actual reference base exists in another module.
  ...
  LogicalResult dfsTraverse(const ReachingDefgraphType &graph) {
    ...
    auto hasCycle = [&](unsigned rootNode, SmallVector<bool> &visited,
      ...
      LLVM_DEBUG(llvm::dbgs()
                     << "\n Root     :"
                     << reachingDefGraph[rootNode].first.getValue().getType()
                     << "," << reachingDefGraph[rootNode].first.getValue()
                     << "," << reachingDefGraph[rootNode].first.getFieldID()
                     << "\n"
                     << getName(reachingDefGraph[rootNode].first););
